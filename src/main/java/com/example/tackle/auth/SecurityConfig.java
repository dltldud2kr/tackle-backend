...
public class SecurityConfig {
     ...
     *  addFilterBefore(new JwtAUthenticationFilter(jwtTokenProvider), UsernamePasswordAuthenticationFilter.class): JWT 인증을 위하여 직접 구현한 필터를
     *  UsernamePasswordAuthenticationFilter 전에 실행하겠다는 설정
     *
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/api/v1/board/info").permitAll()
                .antMatchers("/api/v1/board/list").permitAll()
                .antMatchers("/api/v1/board/search").permitAll()
                // 아래 테스트용
